...
import { Card } from "components/common/News";
import {
  fetchInterviews,
  fetchInterviewsCategory,
  fetchInterviewsByCategoryId,
...
const Interviews = ({ data }) => {
  ...
  const handleCategory = async (id, itemName) => {
    ...
    if (id === 0) {
      ...
      const categoryData = await fetchInterviewsByCategoryId(id);

      setInterviews(categoryData?.data?.data);
              ...
              return (
                <Box key={item.id} mb={4}>
                  ...
                  <Grid container spacing={4}>
                    {interviews
                      .filter((item) => {
                        const [interviewCategory] = item.interview_categories;
                        return categoryId === interviewCategory;
                      })
                      .slice(0, 4)
                      .map((item) => {
                        return (
                          <Grid
                            key={item.id + "-interviews-category-single-page"}
                            item
                            xs={12}
                            sm={6}
                            md={4}
                          ...
                          >
                            <Link href={`/interviews/${item.id}`} passHref>
                              <a
                                style={{
                                  textDecoration: "none",
                                  color: "inherit",
...
export const getServerSideProps = async ({ query }) => {
  const [fetchInterviewsResult, fetchInterviewsCategoryResult] =
    await Promise.all([
      fetchInterviews(query?.page || 1),
      fetchInterviewsCategory(),
    ]);
  ...
  return {
    props: {
      data: {
        ...fetchInterviewsResult.data,
        category: fetchInterviewsCategoryResult.data,
      } || { data: [] },
    },
  };
};
